...
export default function DespachosPage() {
    return (
        <main style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
            ...
            {/* Angulo de sector: ademas de formar, lo que se construye */}
            <section style={{ padding: "4.5rem 0" }}>
                <div className="container" style={{ maxWidth: 900 }}>
                    <span className="kicker-mono">Y además de formar</span>
                    <h2 className="section-title" style={{ textAlign: "left", marginTop: "0.8rem", marginBottom: "0.6rem" }}>
                        Lo que un despacho se puede dejar montado
                    </h2>
                    <p style={{ color: "var(--color-text-muted)", lineHeight: 1.75, maxWidth: 720, margin: "0 0 1.4rem" }}>
                        La formación es la puerta; el ahorro grande llega cuando el despacho además
                        se monta sistemas. Los que más rinden en gestorías, asesorías y abogados:
                    </p>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: "1rem" }}>
                        {[
                            ...
                            <Link key={s.href} href={s.href} style={{ border: "1px solid var(--color-border)", borderRadius: "14px", padding: "1.2rem 1.3rem", background: "var(--color-card-bg, #fff)", display: "block" }}>
                                <h3 style={{ fontFamily: "var(--font-display, serif)", fontSize: "1.1rem", fontWeight: 600, color: "var(--color-text-main)", margin: "0 0 0.35rem", lineHeight: 1.25 }}>{s.titulo} →</h3>
                                <p style={{ color: "var(--color-text-muted)", fontSize: "0.88rem", lineHeight: 1.55, margin: 0 }}>{s.d}</p>
                            </Link>
                        ))}
                    </div>
                </div>
